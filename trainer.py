...
class Trainer(object):
    ...
    def map_labels(self, labels_tensor, mapping_dict):
        new_labels = torch.zeros_like(labels_tensor)

        for old_class, new_class in mapping_dict.items():
            new_labels[labels_tensor == old_class] = new_class
        ...
        for epoch in range(self.epoch, num_epochs):
            ...
            for phase in ['train', 'val']:
                ...
                for i, data in enumerate(dataloaders[phase], 0):
                    ...
                    # Remapping to have only 19 classes
                    labels = self.map_labels(labels, class_mapping)
                    ...
                    with torch.set_grad_enabled(phase == 'train'):
                        ...
                        ious.append(iou)
                        #b ackward + optimize only if in training phase
                        if phase == 'train':
                            loss.backward()
                            self.optim.step()
                            if self.scheduler:
